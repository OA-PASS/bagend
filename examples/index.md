...
> Different organizations often have different identifiers for the same concept, and may not maintain a mapping (or even be aware of) between their respective identifiers.  If BAGEND consumers commit to maintaining and indexing the opaque `identifiers` for BAGEND resources, this increases their discoverability by the institutions producing those resources.
